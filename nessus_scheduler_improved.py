...
def nessus_scan(action: str, scan_id: str):
    """
    requests kütüphanesi kullanarak Nessus API'sine istek gönderir ve sonucu loglar.
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(
                f"Failed to execute '{action}' on scan {scan_id}. "
...
def run_scans(action: str):
    """
    ThreadPoolExecutor kullanarak belirtilen sayıda isteği eşzamanlı olarak çalıştırır.
    """
...
def seconds_until(target_times: list[str], tz: ZoneInfo) -> int:
    """
    Belirtilen zaman dilimine göre bir sonraki hedef zamana kalan saniyeyi hesaplar.
    """
    now = datetime.now(tz)
    min_diff = float('inf')
...
def main():
    ...
    try:
        # Script'in her zaman GMT+3 (Türkiye saati) ile çalışmasını sağlamak için zaman dilimini ayarlıyoruz.
        app_tz = ZoneInfo("Europe/Istanbul")
    except Exception:
        logging.error("Zaman dilimi 'Europe/Istanbul' başlatılamadı. Lütfen sisteminizde zaman dilimi verilerinin kurulu olduğundan emin olun.")
        logging.error("Debian/Ubuntu için: sudo apt-get install tzdata")
        logging.error("CentOS/RHEL için: sudo yum install tzdata")
        return

    logging.info(f"Scheduler started. Waiting for the next scheduled time (Timezone: {app_tz}).")
    while True:
        # Saati alırken her zaman belirlediğimiz zaman dilimini kullanıyoruz.
        now = datetime.now(app_tz)
        now_str = now.strftime("%H:%M")
        
        # Kalan süre hesaplamalarını da bu zaman dilimine göre yapıyoruz.
        secs_to_start = seconds_until(START_TIMES, app_tz)
        secs_to_pause = seconds_until(PAUSE_TIMES, app_tz)
